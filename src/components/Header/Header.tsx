    ...
    <>
      {
        type !== 'null' && (
          <HeaderContainer type={type} isMobile={isMobile} hasSubtitle={!!subtitle}>
            {(doGoBack && location.pathname !== '/dubai-app/welcome') && <ArrowLefContainer onClick={doGoBack}><ArrowLeft /></ArrowLefContainer>}
            <TitleContainer>
              ...
              {type === 'tertiary' && <Link to="http://www.virufy.org/en/" target="_blank"><LogoImg srcSet={`${Logo}, ${Logo2x} 2x, ${Logo3x} 3x`} src={LogoImg} size={location.pathname !== '/dubai-app/welcome' ? 'regular' : 'big'} /></Link>}
            </TitleContainer>
          </HeaderContainer>
        )
      }
